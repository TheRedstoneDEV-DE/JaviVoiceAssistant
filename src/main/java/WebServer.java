		...
		public void run() {
			...
			try {
				...
				if (!requestLine[1].endsWith("?type=resource")) {
					...
					try {
						if (requestLine[1].endsWith("?PlaybackState")) {
							bytesToClient = WebServerThread.state.getBytes();
							WebServerThread.state = "unchanged"; 
							status = "200 OK";
						} else if (requestLine[1].endsWith("?trackInfo")) {
							String line;
							int postDataI = 0;
							...
							if (postDataI > 0) {
								...
								String artist = split[0].replaceAll("title=", "");
								String title = split[1].replaceAll("artist=", "");
								Main.oth.artist=artist;
								Main.oth.title=title;
							}
							status = "200 OK";

						} else if (requestLine[1].endsWith("?command")) {
							String line;
							int postDataI = 0;
							while ((line = in.readLine()) != null && line.length() != 0) {
								if (line.indexOf("Content-Length:") > -1)
							...
							status = "200 OK";
						} else if (requestLine[1].endsWith("?speak")) {
							String line;
							int postDataI = 0;
							while ((line = in.readLine()) != null && line.length() != 0) {
								if (line.indexOf("Content-Length:") > -1)
							...
							if (postDataI > 0) {
								...
								System.out.println(postData);
								String[] split = postData.split("&");
								String command = split[0].replaceAll("words=", "");
								VoiceAssistant.tts.speak(command, 1.0f, false, false);
							}
							status = "200 OK";
						}else if (requestLine[1].endsWith("?reset")) {
							Main.clim.restart=true;
							status = "200 OK";
						}
